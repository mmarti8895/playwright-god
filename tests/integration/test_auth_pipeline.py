...
class TestAuthTemplatesModule:
    def test_get_template_saml_returns_typescript(self):
        ...
        assert "page.waitForURL" in tmpl
    ...
    def test_get_auth_hint_saml(self):
        ...
        assert "SAML" in hint
        assert "page.waitForURL" in hint
...
class TestGenerateWithAuthType:
    ...
    def test_saml_hint_in_prompt(self):
        ...
        assert "page.waitForURL" in prompt

    def test_saml_template_in_prompt(self):
        prompt = self._capture_prompt("saml")
        assert "Reference TypeScript template" in prompt
        assert "signInWithSaml" in prompt
    ...
    def test_oidc_hint_in_prompt(self):
        ...
        assert "OIDC" in prompt or "page.waitForURL" in prompt
...
class TestSAMLPipeline:
    ...
    def test_saml_prompt_contains_typescript_auth_context(self):
        """The prompt sent to the LLM must reference TypeScript auth helpers."""
        captured: list[str] = []
        ...
        assert captured
        assert "signInWithSaml" in captured[0]
        assert "page.waitForURL" in captured[0]
